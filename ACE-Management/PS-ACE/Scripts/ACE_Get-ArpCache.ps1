        ...
        if($SUCCESS -eq 0)
        {
            ...
            for($i = 0; $i -lt $count; $i++)
            {
                ...
                $obj = @{
                    ...
                    PhysicalAddress = [string]([PhysicalAddress]::new($IpNetRow.bPhysAddr).ToString().Insert(2, '-').Insert(5, '-').Insert(8, '-').Insert(11, '-').Insert(14, '-'))
                    IpAddress = [string]([ipaddress]::new($IpNetRow.dwAddr).IPAddressToString)
