function Start-AceScript
{
    ...
    $RoutingKey = New-Object -TypeName System.Text.StringBuilder

    foreach($e in $Enrichment)
    {
        $null = $RoutingKey.Append("$($e).")
    }

    $null = $RoutingKey.Append($($OutputType))
    ...
    foreach($o in (Get-SecurityPackage -ReturnHashtables))
    {
        ...
        $message = ConvertTo-JsonV2 -obj $o
    ...
    $result = Invoke-WebRequest -Method Post -Uri "$($Uri)/ace/result/$($SweepId)" -Body (ConvertTo-Json $output) -Headers $header -ContentType application/json
...
$Mod = New-InMemoryModule -ModuleName GetSecurityPackage

$SECPKG_FLAG = psenum $Mod SECPKG_FLAG UInt32 @{
    INTEGRITY                = 0x1
    PRIVACY                  = 0x2
    TOKEN_ONLY               = 0x4
    DATAGRAM                 = 0x8
...
$SecPkgInfo = struct $Mod SecPkgInfo @{
...
$Types = $FunctionDefinitions | Add-Win32Type -Module $Mod -Namespace GetSecurityPackage
